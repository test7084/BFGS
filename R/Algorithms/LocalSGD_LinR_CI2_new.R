...
local_sgd <- function(X, y, true_w, num_clients, initial_model, true_covariance, max_iter = 500, 
  ...
  for (iteration in 1:max_iter) {
    gradients <- vector("list", num_clients)
    ...
    for (m in 1:num_clients) {
      ...
      end_idx <- (start_idx + batch_size - 1) %% nrow(X_m) + 1
      ...
      result <- loss_and_gradient(local_models[[m]], X_batch, y_batch)
      local_losses[m] <- result$loss
      gradients[[m]] <- result$gradient
    ...
    g_outer_avg <- Reduce("+", lapply(gradients, function(g) g %*% t(g))) / num_clients
    global_hessian <- diag(p)  # 使用单位矩阵作为海森矩阵
    aveT_H_global_inv_w <- solve(global_hessian, w)
    est_var <- t(aveT_H_global_inv_w) %*% g_outer_avg %*% aveT_H_global_inv_w
    
    number_samples <- num_clients * batch_size
    ci_length <- z_975 * sqrt(est_var / number_samples)
    ...
    cat(sprintf("Local SGD %d: Loss = %.4f, L2 Norm = %.4f, CP = %.4f, final_CP = %.1f, AIL = %.4f\n",
